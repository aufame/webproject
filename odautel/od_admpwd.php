...
{ $pwdold=trim($_POST["pwdold"]);
	...
	$res=$conn->query("select id from `od_admin` where username='$AdminUsernme' and password=md5('$pwdold')",PDO::FETCH_NUM); 
	if($res && $res->fetchColumn(0))
	{ $conn->exec("update `od_admin` set password=md5('$pwdnew') where username='$AdminUsernme'");
	  PageReturn("密码修改成功，请用新密码重新登录！",0);	
	}
  else PageReturn("原密码错误".$pwdold);
}	
?>
<html>
<head>
	...
	<table width="250" border="2" align="center" cellpadding="1" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#CCCCCC">
        <form method="post" action="?action=save" onsubmit="return FormCheck(this);">
          <tr bgcolor="#FFFFFF"> 
            ...
            <td width="150" align="center"><font color=red><?php echo $AdminUsernme;?></font></td>
          </tr>
          <tr bgcolor="#FFFFFF"> 
