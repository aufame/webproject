<?php require("includes/dbconn.php");
 if(!CheckLogin(0)){
   echo ' Not login! <a href="leftmenu.php">[Refresh]</a>';
   exit(0);
 }
 OpenDB();
 $res_menu=$conn->query("select * from `od_popedom` where parent=0 order by sort",PDO::FETCH_ASSOC);
?>
<html>
<head>
...
function sethome(achor)
{  document.cookie = "odautel_home="+ escape(achor.href);
}
</SCRIPT>
   ...
   <TD valign="top">
      ...
      <TR>
         <TD class="menu_title_normal"  background="images/title_bg_quit.gif"  height=25 align="center">
         	 <a href="/" target="_blank" ><B>前台首页</B></A> | <A href="admlogout.php"><B>退出</B>&nbsp;&nbsp;&nbsp;</A>
         </TD>
      </TR>
      </TABLE>
<!----------------------------------------------------------------------------------------->      
      ...
      foreach($res_sub as $row_sub){
        if(CheckPopedom($row_sub["id"])){
          $menu_count=$menu_count+1;
       	  $menu_items=$menu_items."<TR><TD class='menubar'><a href='{$row_sub['path']}' onclick='sethome(this)'>{$row_sub['title']}</a></TD></TR>";
       	}
      }
      if($menu_count>0){
       ...
       <TR>
         <TD>
           <DIV class="sec_menu" id="menu_0"  style="FILTER: alpha(Opacity=100); WIDTH: 158px; HEIGHT:30px">
              ...
              <TR>
                <TD align="center" height="20" style="line-height:120%"><font color="#FF0000"><?php echo $AdminUsernme;?></font><font color="#6f6f6f"></font>@欧达电子</TD>
              </TR>
              </TABLE>
           </DIV>
         </TD>
