 ...
 if(($mode=@$_GET['mode'])){
   if($mode=='del'){
     ...
     if(is_numeric($meuid) && $meuid>0){
       $sub_menus=$conn->query('select count(*) from od_popedom where parent='.$meuid)->fetchColumn(0);
       if($sub_menus) PageReturn("菜单项删除失败，存在非空子菜单！");
       $conn->exec("update `od_popedom` set parent=-1 where id=$meuid or parent=$meuid");
       PageReturn("菜单项删除成功！");
     }
   }  
   else if($mode=='update'){
     ...
     if(is_numeric($meuID) && is_numeric($meuSort)){
       ...
       $conn->exec("update `od_popedom` set title='$menu_title',sort=$meuSort,path='$menu_path',remark='$menu_remark' where id=$meuID");
       PageReturn("保存成功！");
     }
   }
   else if($mode=='add'){
     ...
     if(is_numeric($parent) && $menu_title){
       $max_sort=$conn->query("select max(sort) from `od_popedom` where parent=$parent")->fetchColumn(0);
       $max_sort=($max_sort)?(int)$max_sort+1:1;
       $freeid=$conn->query("select id from `od_popedom` where parent=-1 limit 1")->fetchColumn(0); $sql="`od_popedom` set parent=$parent,title='$menu_title',sort=$max_sort,path='',remark=''";
     ...
     if(is_numeric($menuid) && is_numeric($parent)){
       $conn->exec("update `od_popedom` set parent=$parent where id=$menuid");
...
function MoveMenu(myForm){
  ...
  $res=$conn->query('select id,sort,title from od_popedom where parent=0 order by sort asc',PDO::FETCH_NUM);
  ...
  ShowDialog("选择父级菜单",html,150,200,onSelectParent);
}
...
<table width="100%" border="5" align="center" cellpadding="5" cellspacing="5" bordercolor="#CCCCCC" bgcolor="#FFFFFF">
  ...
  <tr> 
    ...
    $res_sub=$conn->query("select * from `od_popedom` where parent={$menu_root['id']} order by sort",PDO::FETCH_ASSOC);
