...
{ var querystr=window.location.search;   //location.search是从当前URL的?号开始的字符串
...
{ var querystr=window.location.href;   //location.search是从当前URL的?号开始的字符串
...
{ var outtext="&nbsp;&nbsp;用户:<input name='username' type='text' maxlength='32' style='height:20px;width:90px'>";
      outtext+="&nbsp;密码:<input name='password' type='password' maxlength='32' style='height:20px;width:90px'>"
      outtext+="&nbsp;验证:<input name='verifycode' type='text'  maxlength='4' style='height:20px;width:60px;'>"
      ...
      outtext+="&nbsp; <input type='button' value='注册' onClick=\"location.href='reg.asp'\">";
...
{ var online_userid = getCookie("shopuser");
  ...
  { var ret= PostForm("","login.asp?mode=getinfo&userid="+online_userid);
    ...
    { document.getElementById("loginfo").innerHTML=ret;
    	if(ret.charAt(0)=="&") FriendUserOnline=true;
    }
...
{ var qqInfoLen=QQs.length;
	...
	{ if(qqInfoLen>0 && QQs[0]!="" ) 
	  { var QQNick,qqNumber,i,ServiceCode="客 服 QQ：";
      ...
      { if(i%8==0 && i>0)ServiceCode+="<br>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; ";
   	    if(randomoffet>=qqInfoLen)randomoffet=0;
		    ...
		    ServiceCode+="<A title='QQ在线客服:"+qqNumber+"' href='http://wpa.qq.com/msgrd?V=1&Uin="+qqNumber+"&Site="+webname+"&Menu=yes' target='_blank'><IMG src='http://wpa.qq.com/pa?p=1:"+qqNumber+":4' border=0>"+QQNick+"</A>&nbsp;&nbsp;";
	    }
      ServiceCode+="<br>";
    }
    else
...
function ShowFriendPrice(money)
{ if(FriendUserOnline) document.write(money);
	else document.write("非等级查看");
}
